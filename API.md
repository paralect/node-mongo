...
- `options: Object` - optional object with options of the service.
  - `addCreatedOnField: Boolean` - if `true`, we add the `createdOn` field for each document to be created using the [create](#create) method, the default value is `true`.
  - `addUpdatedOnField: Boolean` - if `true`, we add and update the `updatedOn` field for each document to be updated using [updateOne](#updateone) or [updateMany](#updatemany) methods, the default value is `true`.
  - `useStringId: Boolean` - if `true`, we replace `_id` ([ObjectId](https://docs.mongodb.com/manual/reference/method/ObjectId/) by default) with a string that is generated using the [generateId](#generateid) method, default value is `true`.
